    ...
    if (!question.bonusPaid && question.bonusPending) {
        ...
        if (count) {
            ...
            if (totalReputation) {
                ...
                Object.keys(count.reputationDestribution).forEach((userId) => {
                    const coins =
                        count.reputationDestribution[userId] * priceCoins
            ...
            await Promise.all(
                Object.keys(distributionCoins).map(async (userId) => {
                    ...
                    const result = await Account.updateOne(
                        { _id: userId },
                        {
                            $inc: { wallet: distributionCoins[userId] },
                            $push: {
                                gains: {
                                    $each: [
                                        {
                                            coins: distributionCoins[userId],
                                            actionType: 'bonus',
