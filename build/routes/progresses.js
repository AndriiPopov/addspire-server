...
router.get('/:ownerId/:id', authNotForce, async (req, res, next) => {
  try {
    ...
    let progress = await Progress.findById(progressId).select('-patch').lean().exec();
    let account;
    ...
    if (progress) {
      let accountIds = [progress.owner, progress.worker, ...progress.goal.supporters, ...progress.goal.experts, ...account.friends.map(item => item.friend)];
...
router.post('/add', auth, async (req, res) => {
  try {
    ...
    if (req.user) {
      account = await getAccount(req, res, 'friends goals progresses currentId', true);
    }
    ...
    if (ownerId === account._id || userId === account._id) {
      ...
      if (userId === account._id) {
        ...
        user = await Account.findById(userId).select('friends goals progresses currentId').exec();
      }

      if (ownerId === account._id) {
        owner = account;
      } else {
        owner = await Account.findById(ownerId).select('friends goals progresses currentId').exec();
      }
      ...
      if (user && owner) {
        ...
        if (goal) {
          const existingProgressesIds = goal.progresses.filter(progress => progress.userId === userId).map(progress => progress.userId);
          const existingProgressesObjs = await Progress.find({
            _id: {
              $in: existingProgressesIds
            }
          }).select('status').lean().exec();

          if (!existingProgressesObjs.find(item => item.status === 'not started')) {
            const {
              progresses,
              ...progressValue
            } = goal.toObject();
            let progress = new Progress({
              status: userId !== ownerId ? 'not started' : 'in progress',
              worker: user._id,
              owner: owner._id,
              goal: progressValue,
              ...
              stages: [{
                milestoneId: 'start',
                approvedBy: owner._id === user._id ? [] : [{
                  accountId: account._id === user._id ? user._id : owner._id
                }]
              }]
            });
            ...
            const expertsAndSupporters = [...new Set([...goal.experts, ...goal.supporters])].filter(item => item !== ownerId && item !== userId);
            const ggg = await Account.updateMany({
              _id: {
                $in: expertsAndSupporters
              }
            }, {
              $push: {
