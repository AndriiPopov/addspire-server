...
router.get('/:_id/:wishlistItemId', authNotForce, async (req, res, next) => {
  try {
    let profile = await Account.findById(req.params._id).select({
      wishlist: {
        $elemMatch: {
          itemId: req.params.wishlistItemId
        }
      }
    }).lean().exec();
    let account;
...
router.post('/add', auth, async (req, res) => {
  try {
    ...
    if (req.user) {
      account = await getAccount(req, res, 'name wishlist currentId', true);
    }
    ...
    if (wishlistItemId) {
      account.wishlist = account.toObject().wishlist.map(item => {
        if (item.itemId === wishlistItemId) return { ...item,
          ...req.body.value
        };else return item;
      });
    } else {
...
router.post('/delete/:id', auth, async (req, res) => {
  try {
    ...
    if (itemId) {
      account.wishlist = account.toObject().wishlist.filter(item => item.itemId !== itemId);
    }
